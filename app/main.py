...
def _ensure_runtime_compatibility_schema() -> None:
    ...
    with engine.begin() as conn:
        ...
        if "panic_events" in table_names:
            ...
            _add_column_if_missing(conn, columns, "panic_events", "responder_user_ids_json", "TEXT DEFAULT '[]'")
            _add_column_if_missing(conn, columns, "panic_events", "responder_details_json", "TEXT DEFAULT '[]'")
            _add_column_if_missing(conn, columns, "panic_events", "ignored_user_ids_json", "TEXT DEFAULT '[]'")
            _add_column_if_missing(conn, columns, "panic_events", "false_report_user_ids_json", "TEXT DEFAULT '[]'")
            _add_column_if_missing(conn, columns, "panic_events", "trigger_trust_score", "INTEGER DEFAULT 0")
            _add_column_if_missing(conn, columns, "panic_events", "incident_notes", "TEXT")
            _add_column_if_missing(conn, columns, "panic_events", "response_started_at", datetime_sql)
            _add_column_if_missing(conn, columns, "panic_events", "last_responder_at", datetime_sql)
...
def _ensure_homeowner_settings_schema() -> None:
    inspector = inspect(engine)
    table_names = set(inspector.get_table_names())
    if "resident_settings" not in table_names:
        Base.metadata.tables["resident_settings"].create(bind=engine, checkfirst=True)
